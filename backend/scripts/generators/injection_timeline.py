...
.track-wrap{position:relative;margin:0 24px 36px}
.track{height:3px;background:#1e1e2e;border-radius:2px}
.fill{height:100%;background:linear-gradient(90deg,#4f46e5,#e87f3a);border-radius:2px;
  width:55%;transition:width .4s}
.now{position:absolute;left:55%;transform:translateX(-50%);top:-22px;
  font-size:8px;color:#e87f3a;text-transform:uppercase;letter-spacing:.12em;white-space:nowrap;
  display:flex;flex-direction:column;align-items:center;gap:2px}
.now::after{content:'';width:1px;height:10px;background:#e87f3a;display:block}
...
.phase-title{font-size:12px;font-weight:500;margin-bottom:4px;line-height:1.3}
.phase-sub{font-size:10px;color:#9ca3af;line-height:1.4}
.phase-body{max-height:0;overflow:hidden;transition:max-height .4s ease,opacity .3s;
  opacity:0;font-size:10px;color:#6b7280;line-height:1.6;margin-top:0}
.phase.active .phase-body{max-height:160px;opacity:1;margin-top:8px}
...
.dot.current{background:#e87f3a;border-color:#e87f3a;
  box-shadow:0 0 0 3px #e87f3a22,0 0 8px #e87f3a88}
...
const data=[
  ...
  },
  {
    ...
    dotState:'current',
    body:'Le RLHF, le fine-tuning, les retours humains injectent en continu de la valeur, du jugement, de la nuance. Le modèle apprend la forme de la division correcte sans encore pouvoir l\'exécuter seul. Progressivement, la forme s\'imprime en elle.'
  },
  {
